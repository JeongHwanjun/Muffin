...
public class ComboRule : ScriptableObject
{
    ...
    [Tooltip("모두 포함되어야 발동하는 재료들(AND)")] public List<IngredientBase> requireAllIngredients = new();
    [Tooltip("하나 이상 포함되면 발동하는 재료들(OR)")] public List<IngredientBase> requireAnyIngredients = new();
    [Tooltip("조건 만족시 스탯 변화량(음수 가능)")] public IngredientBase delta; // 콤보에 의한 스탯 변화량
